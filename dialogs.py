...
def show_usage_dialog(parent, is_dark=False):
    ...
    html = (
        ...
        "<b>GSM 调试助手：</b>GSM/Modem 模块 AT 指令调试<br>"
        "  设备信息查询（IMEI/ICCID/本机号码/信号强度等）<br>"
        "  短信管理（读取/发送/删除）、通话控制（拨打/接听/挂断）<br>"
        "  状态监控（信号/SIM/网络/运营商实时显示）<br>"
        "<b>自动应答：</b>接收到特定数据时自动发送预设响应<br>"
        "  支持 HEX 匹配 / 文本包含 / 文本完全匹配三种模式<br>"
        "  可配置响应延迟、最大次数、多规则排序与启用/禁用<br>"
        "<b>Modbus 工具：</b><br>"
...
def show_about_dialog(parent, is_dark=False):
    ...
    msg = QMessageBox(QMessageBox.NoIcon, "关于",
        ...
        "亮色/暗黑双主题、ANSI 转义码解析、<br>"
        "GSM/Modem AT 指令调试、自动应答等功能。<br><br>"
        "<b>作者:</b> GAOXIANG<br>"
